 ...
 * @param s1 
   ...
   **/   
   ...
   {
      if (s1 == null || s2 == null) {
         throw new NullPointerException("Input Statisticians must not be null.");
     }
      Statistician combined = new Statistician();

      
     
      //legnth, mean and sum operations.
      combined.length = s1.length +s2.length;
      combined.mean = (s1.mean()+s2.mean())/2;
      combined.sum = s1.sum() + s2.sum();

      //largest number
      if(Double.isNaN(s1.largest)){
         combined.largest = s2.largest;
      }
      else if(Double.isNaN(s2.largest) ){
         combined.largest = s1.largest;
      }
      else {
         combined.largest = Math.max(s1.maximum(),s2.maximum());
      }


      //smallest number
      if(Double.isNaN(s2.smallest)){
         combined.smallest = s1.smallest;
      }
      else if(Double.isNaN(s1.smallest)){
         combined.smallest = s2.smallest;
      }
      else {
         combined.smallest = Math.min(s1.minimum(),s2.minimum());
      }

      

      //last number of the inputs
      if(Double.isNaN(s1.lastnum)){
         combined.lastnum = s2.last();
      }
      else if(Double.isNaN(s2.lastnum)){
         combined.lastnum = s1.lastnum;
      }
      else {
         combined.lastnum = s2.lastnum;
      } 
